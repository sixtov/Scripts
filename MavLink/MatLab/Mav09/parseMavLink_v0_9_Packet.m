function S = parseMavLink_v0_9_Packet(p)
	...
	switch(messid)
		...
		case 113
			...
			S.CDNR_CONTROLLER = parse_CDNR_CONTROLLER_v0_9(p);
		...
		case 0
			...
			%% the autopilot).
			S.HEARTBEAT = parse_HEARTBEAT_v0_9(p);
		...
		case 3
			...
			S.PING = parse_PING_v0_9(p);

		case 4
			%% UTC date and time from GPS module
			S.SYSTEM_TIME_UTC = parse_SYSTEM_TIME_UTC_v0_9(p);

		case 5
			%% Request to control this MAV
			S.CHANGE_OPERATOR_CONTROL = parse_CHANGE_OPERATOR_CONTROL_v0_9(p);

		case 6
			%% Accept / deny control of this MAV
			S.CHANGE_OPERATOR_CONTROL_ACK = parse_CHANGE_OPERATOR_CONTROL_ACK_v0_9(p);
		...
		case 7
			...
			%% for true safety.
			S.AUTH_KEY = parse_AUTH_KEY_v0_9(p);
		...
		case 9
			...
			S.ACTION_ACK = parse_ACTION_ACK_v0_9(p);
		...
		case 11
			...
			S.SET_MODE = parse_SET_MODE_v0_9(p);
		...
		case 12
			...
			%% components.
			S.SET_NAV_MODE = parse_SET_NAV_MODE_v0_9(p);
		...
		case 20
			...
			S.PARAM_REQUEST_READ = parse_PARAM_REQUEST_READ_v0_9(p);

		case 21
			%% Request all parameters of this component. After his request, all parameters are 
			%% emitted.
			S.PARAM_REQUEST_LIST = parse_PARAM_REQUEST_LIST_v0_9(p);
		...
		case 22
			...
			S.PARAM_VALUE = parse_PARAM_VALUE_v0_9(p);
		...
		case 23
			...
			S.PARAM_SET = parse_PARAM_SET_v0_9(p);
		...
		case 27
			...
			S.GPS_STATUS = parse_GPS_STATUS_v0_9(p);
		...
		case 28
			...
			%% debugging.
			S.RAW_IMU = parse_RAW_IMU_v0_9(p);
		...
		case 29
			...
			%% values.
			S.RAW_PRESSURE = parse_RAW_PRESSURE_v0_9(p);
		...
		case 38
			...
			S.SCALED_PRESSURE = parse_SCALED_PRESSURE_v0_9(p);

		case 30
			%% The attitude in the aeronautical frame (right-handed, Z-down, X-front, Y-right).
			S.ATTITUDE = parse_ATTITUDE_v0_9(p);
		...
		case 32
			...
			%% message GLOBAL_POSITION for the global position estimate. Coordinate frame is 
			%% right-handed, Z-axis up (GPS frame)
			S.GPS_RAW = parse_GPS_RAW_v0_9(p);
		...
		case 34
			...
			S.SYS_STATUS = parse_SYS_STATUS_v0_9(p);
		...
		case 35
			...
			S.RC_CHANNELS_RAW = parse_RC_CHANNELS_RAW_v0_9(p);

		case 36
			%% The scaled values of the RC channels received. (-100%) -10000, (0%) 0, (100%) 
			%% 10000
			S.RC_CHANNELS_SCALED = parse_RC_CHANNELS_SCALED_v0_9(p);
		...
		case 37
			...
			S.SERVO_OUTPUT_RAW = parse_SERVO_OUTPUT_RAW_v0_9(p);
		...
		case 39
			...
			S.WAYPOINT = parse_WAYPOINT_v0_9(p);
		...
		case 40
			...
			S.WAYPOINT_REQUEST = parse_WAYPOINT_REQUEST_v0_9(p);
		...
		case 44
			...
			%% of waypoints.
			S.WAYPOINT_COUNT = parse_WAYPOINT_COUNT_v0_9(p);

		case 45
			%% Delete all waypoints at once.
			S.WAYPOINT_CLEAR_ALL = parse_WAYPOINT_CLEAR_ALL_v0_9(p);
		...
		case 46
			...
			%% next waypoint.
			S.WAYPOINT_REACHED = parse_WAYPOINT_REACHED_v0_9(p);
		...
		case 47
			...
			S.WAYPOINT_ACK = parse_WAYPOINT_ACK_v0_9(p);
		...
		case 48
			...
			S.GPS_SET_GLOBAL_ORIGIN = parse_GPS_SET_GLOBAL_ORIGIN_v0_9(p);

		case 49
			%% Once the MAV sets a new GPS-Local correspondence, this message announces the origin 
			%% (0,0,0) position
			S.GPS_LOCAL_ORIGIN_SET = parse_GPS_LOCAL_ORIGIN_SET_v0_9(p);
		...
		case 50
			...
			S.LOCAL_POSITION_SETPOINT_SET = parse_LOCAL_POSITION_SETPOINT_SET_v0_9(p);

		case 51
			%% Transmit the current local setpoint of the controller to other MAVs (collision avoidance) 
			%% and to the GCS.
			S.LOCAL_POSITION_SETPOINT = parse_LOCAL_POSITION_SETPOINT_v0_9(p);

		case 52
			S.CONTROL_STATUS = parse_CONTROL_STATUS_v0_9(p);
		...
		case 53
			...
			S.SAFETY_SET_ALLOWED_AREA = parse_SAFETY_SET_ALLOWED_AREA_v0_9(p);

		case 54
			%% Read out the safety zone the MAV currently assumes.
			S.SAFETY_ALLOWED_AREA = parse_SAFETY_ALLOWED_AREA_v0_9(p);

		case 55
			%% Set roll, pitch and yaw.
			S.SET_ROLL_PITCH_YAW_THRUST = parse_SET_ROLL_PITCH_YAW_THRUST_v0_9(p);

		case 56
			%% Set roll, pitch and yaw.
			S.SET_ROLL_PITCH_YAW_SPEED_THRUST = parse_SET_ROLL_PITCH_YAW_SPEED_THRUST_v0_9(p);

		case 57
			%% Setpoint in roll, pitch, yaw currently active on the system.
			S.ROLL_PITCH_YAW_THRUST_SETPOINT = parse_ROLL_PITCH_YAW_THRUST_SETPOINT_v0_9(p);

		case 58
			%% Setpoint in rollspeed, pitchspeed, yawspeed currently active on the system.
			S.ROLL_PITCH_YAW_SPEED_THRUST_SETPOINT = parse_ROLL_PITCH_YAW_SPEED_THRUST_SETPOINT_v0_9(p);
		...
		case 62
			...
			S.NAV_CONTROLLER_OUTPUT = parse_NAV_CONTROLLER_OUTPUT_v0_9(p);
		...
		case 63
			...
			S.POSITION_TARGET = parse_POSITION_TARGET_v0_9(p);
		...
		case 64
			...
			S.STATE_CORRECTION = parse_STATE_CORRECTION_v0_9(p);

		case 65
			S.SET_ALTITUDE = parse_SET_ALTITUDE_v0_9(p);

		case 66
			S.REQUEST_DATA_STREAM = parse_REQUEST_DATA_STREAM_v0_9(p);

		case 67
			%% This packet is useful for high throughput                 applications such as hardware 
			%% in the loop simulations.             
			S.HIL_STATE = parse_HIL_STATE_v0_9(p);

		case 68
			%% Hardware in the loop control outputs
			S.HIL_CONTROLS = parse_HIL_CONTROLS_v0_9(p);

		case 69
			S.MANUAL_CONTROL = parse_MANUAL_CONTROL_v0_9(p);
		...
		case 70
			...
			S.RC_CHANNELS_OVERRIDE = parse_RC_CHANNELS_OVERRIDE_v0_9(p);

		case 73
			%% The filtered global position (e.g. fused GPS and accelerometers). The position is 
			%% in GPS-frame (right-handed, Z-up)
			S.GLOBAL_POSITION_INT = parse_GLOBAL_POSITION_INT_v0_9(p);

		case 74
			%% Metrics typically displayed on a HUD for fixed wing aircraft
			S.VFR_HUD = parse_VFR_HUD_v0_9(p);

		case 75
			%% Send a command with up to four parameters to the MAV
			S.COMMAND = parse_COMMAND_v0_9(p);

		case 76
			%% Report status of a command. Includes feedback wether the command was executed
			S.COMMAND_ACK = parse_COMMAND_ACK_v0_9(p);

		case 100
			%% Optical flow from a flow sensor (e.g. optical mouse sensor)
			S.OPTICAL_FLOW = parse_OPTICAL_FLOW_v0_9(p);

		case 140
			%% Object has been detected
			S.OBJECT_DETECTION_EVENT = parse_OBJECT_DETECTION_EVENT_v0_9(p);

		case 251
			S.DEBUG_VECT = parse_DEBUG_VECT_v0_9(p);
		...
		case 252
			...
			%% debug output.
			S.NAMED_VALUE_FLOAT = parse_NAMED_VALUE_FLOAT_v0_9(p);

		case 253
			%% Send a key-value pair as integer. The use of this message is discouraged for normal 
			%% packets, but a quite efficient way for testing new messages and getting experimental 
			%% debug output.
			S.NAMED_VALUE_INT = parse_NAMED_VALUE_INT_v0_9(p);
		...
		case 254
			...
			S.STATUSTEXT = parse_STATUSTEXT_v0_9(p);
		...
		case 255
			...
			S.DEBUG = parse_DEBUG_v0_9(p);

		case 150
			%% Offsets and calibrations values for hardware         sensors. This makes it easier 
			%% to debug the calibration process.
			S.SENSOR_OFFSETS = parse_SENSOR_OFFSETS_v0_9(p);

		case 151
			%% set the magnetometer offsets
			S.SET_MAG_OFFSETS = parse_SET_MAG_OFFSETS_v0_9(p);

		case 152
			%% state of APM memory
			S.MEMINFO = parse_MEMINFO_v0_9(p);

		case 153
			%% raw ADC output
			S.AP_ADC = parse_AP_ADC_v0_9(p);

		case 154
			%% Configure on-board Camera Control System.
			S.DIGICAM_CONFIGURE = parse_DIGICAM_CONFIGURE_v0_9(p);

		case 155
			%% Control on-board Camera Control System to take shots.
			S.DIGICAM_CONTROL = parse_DIGICAM_CONTROL_v0_9(p);

		case 156
			%% Message to configure a camera mount, directional antenna, etc.
			S.MOUNT_CONFIGURE = parse_MOUNT_CONFIGURE_v0_9(p);

		case 157
			%% Message to control a camera mount, directional antenna, etc.
			S.MOUNT_CONTROL = parse_MOUNT_CONTROL_v0_9(p);

		case 158
			%% Message with some status from APM to GCS about camera or antenna mount
			S.MOUNT_STATUS = parse_MOUNT_STATUS_v0_9(p);

		case 160
			%% A fence point. Used to set a point when from        GCS -> MAV. Also used to return 
			%% a point from MAV -> GCS
			S.FENCE_POINT = parse_FENCE_POINT_v0_9(p);

		case 161
			%% Request a current fence point from MAV
			S.FENCE_FETCH_POINT = parse_FENCE_FETCH_POINT_v0_9(p);

		case 162
			%% Status of geo-fencing. Sent in extended      status stream when fencing enabled
			S.FENCE_STATUS = parse_FENCE_STATUS_v0_9(p);
