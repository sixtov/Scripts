function p = encodeMavLink_v0_9_Packet(ID,S)
	switch(ID)
		...
		case 113
			...
			p = encode_CDNR_CONTROLLER_v0_9(S.CDNR_CONTROLLER);
		...
		case 0
			...
			%% the autopilot).
			p = encode_HEARTBEAT_v0_9(S.HEARTBEAT);
		...
		case 3
			...
			p = encode_PING_v0_9(S.PING);

		case 4
			%% UTC date and time from GPS module
			p = encode_SYSTEM_TIME_UTC_v0_9(S.SYSTEM_TIME_UTC);

		case 5
			%% Request to control this MAV
			p = encode_CHANGE_OPERATOR_CONTROL_v0_9(S.CHANGE_OPERATOR_CONTROL);

		case 6
			%% Accept / deny control of this MAV
			p = encode_CHANGE_OPERATOR_CONTROL_ACK_v0_9(S.CHANGE_OPERATOR_CONTROL_ACK);
		...
		case 7
			...
			%% for true safety.
			p = encode_AUTH_KEY_v0_9(S.AUTH_KEY);
		...
		case 9
			...
			p = encode_ACTION_ACK_v0_9(S.ACTION_ACK);
		...
		case 11
			...
			p = encode_SET_MODE_v0_9(S.SET_MODE);
		...
		case 12
			...
			%% components.
			p = encode_SET_NAV_MODE_v0_9(S.SET_NAV_MODE);
		...
		case 20
			...
			p = encode_PARAM_REQUEST_READ_v0_9(S.PARAM_REQUEST_READ);

		case 21
			%% Request all parameters of this component. After his request, all parameters are 
			%% emitted.
			p = encode_PARAM_REQUEST_LIST_v0_9(S.PARAM_REQUEST_LIST);
		...
		case 22
			...
			p = encode_PARAM_VALUE_v0_9(S.PARAM_VALUE);
		...
		case 23
			...
			p = encode_PARAM_SET_v0_9(S.PARAM_SET);
		...
		case 27
			...
			p = encode_GPS_STATUS_v0_9(S.GPS_STATUS);
		...
		case 28
			...
			%% debugging.
			p = encode_RAW_IMU_v0_9(S.RAW_IMU);
		...
		case 29
			...
			%% values.
			p = encode_RAW_PRESSURE_v0_9(S.RAW_PRESSURE);
		...
		case 38
			...
			p = encode_SCALED_PRESSURE_v0_9(S.SCALED_PRESSURE);

		case 30
			%% The attitude in the aeronautical frame (right-handed, Z-down, X-front, Y-right).
			p = encode_ATTITUDE_v0_9(S.ATTITUDE);
		...
		case 32
			...
			%% message GLOBAL_POSITION for the global position estimate. Coordinate frame is 
			%% right-handed, Z-axis up (GPS frame)
			p = encode_GPS_RAW_v0_9(S.GPS_RAW);
		...
		case 34
			...
			p = encode_SYS_STATUS_v0_9(S.SYS_STATUS);
		...
		case 35
			...
			p = encode_RC_CHANNELS_RAW_v0_9(S.RC_CHANNELS_RAW);

		case 36
			%% The scaled values of the RC channels received. (-100%) -10000, (0%) 0, (100%) 
			%% 10000
			p = encode_RC_CHANNELS_SCALED_v0_9(S.RC_CHANNELS_SCALED);
		...
		case 37
			...
			p = encode_SERVO_OUTPUT_RAW_v0_9(S.SERVO_OUTPUT_RAW);
		...
		case 39
			...
			p = encode_WAYPOINT_v0_9(S.WAYPOINT);
		...
		case 40
			...
			p = encode_WAYPOINT_REQUEST_v0_9(S.WAYPOINT_REQUEST);
		...
		case 44
			...
			%% of waypoints.
			p = encode_WAYPOINT_COUNT_v0_9(S.WAYPOINT_COUNT);

		case 45
			%% Delete all waypoints at once.
			p = encode_WAYPOINT_CLEAR_ALL_v0_9(S.WAYPOINT_CLEAR_ALL);
		...
		case 46
			...
			%% next waypoint.
			p = encode_WAYPOINT_REACHED_v0_9(S.WAYPOINT_REACHED);
		...
		case 47
			...
			p = encode_WAYPOINT_ACK_v0_9(S.WAYPOINT_ACK);
		...
		case 48
			...
			p = encode_GPS_SET_GLOBAL_ORIGIN_v0_9(S.GPS_SET_GLOBAL_ORIGIN);

		case 49
			%% Once the MAV sets a new GPS-Local correspondence, this message announces the origin 
			%% (0,0,0) position
			p = encode_GPS_LOCAL_ORIGIN_SET_v0_9(S.GPS_LOCAL_ORIGIN_SET);
		...
		case 50
			...
			p = encode_LOCAL_POSITION_SETPOINT_SET_v0_9(S.LOCAL_POSITION_SETPOINT_SET);

		case 51
			%% Transmit the current local setpoint of the controller to other MAVs (collision avoidance) 
			%% and to the GCS.
			p = encode_LOCAL_POSITION_SETPOINT_v0_9(S.LOCAL_POSITION_SETPOINT);

		case 52
			p = encode_CONTROL_STATUS_v0_9(S.CONTROL_STATUS);
		...
		case 53
			...
			p = encode_SAFETY_SET_ALLOWED_AREA_v0_9(S.SAFETY_SET_ALLOWED_AREA);

		case 54
			%% Read out the safety zone the MAV currently assumes.
			p = encode_SAFETY_ALLOWED_AREA_v0_9(S.SAFETY_ALLOWED_AREA);

		case 55
			%% Set roll, pitch and yaw.
			p = encode_SET_ROLL_PITCH_YAW_THRUST_v0_9(S.SET_ROLL_PITCH_YAW_THRUST);

		case 56
			%% Set roll, pitch and yaw.
			p = encode_SET_ROLL_PITCH_YAW_SPEED_THRUST_v0_9(S.SET_ROLL_PITCH_YAW_SPEED_THRUST);

		case 57
			%% Setpoint in roll, pitch, yaw currently active on the system.
			p = encode_ROLL_PITCH_YAW_THRUST_SETPOINT_v0_9(S.ROLL_PITCH_YAW_THRUST_SETPOINT);

		case 58
			%% Setpoint in rollspeed, pitchspeed, yawspeed currently active on the system.
			p = encode_ROLL_PITCH_YAW_SPEED_THRUST_SETPOINT_v0_9(S.ROLL_PITCH_YAW_SPEED_THRUST_SETPOINT);
		...
		case 62
			...
			p = encode_NAV_CONTROLLER_OUTPUT_v0_9(S.NAV_CONTROLLER_OUTPUT);
		...
		case 63
			...
			p = encode_POSITION_TARGET_v0_9(S.POSITION_TARGET);
		...
		case 64
			...
			p = encode_STATE_CORRECTION_v0_9(S.STATE_CORRECTION);

		case 65
			p = encode_SET_ALTITUDE_v0_9(S.SET_ALTITUDE);

		case 66
			p = encode_REQUEST_DATA_STREAM_v0_9(S.REQUEST_DATA_STREAM);

		case 67
			%% This packet is useful for high throughput                 applications such as hardware 
			%% in the loop simulations.             
			p = encode_HIL_STATE_v0_9(S.HIL_STATE);

		case 68
			%% Hardware in the loop control outputs
			p = encode_HIL_CONTROLS_v0_9(S.HIL_CONTROLS);

		case 69
			p = encode_MANUAL_CONTROL_v0_9(S.MANUAL_CONTROL);
		...
		case 70
			...
			p = encode_RC_CHANNELS_OVERRIDE_v0_9(S.RC_CHANNELS_OVERRIDE);

		case 73
			%% The filtered global position (e.g. fused GPS and accelerometers). The position is 
			%% in GPS-frame (right-handed, Z-up)
			p = encode_GLOBAL_POSITION_INT_v0_9(S.GLOBAL_POSITION_INT);

		case 74
			%% Metrics typically displayed on a HUD for fixed wing aircraft
			p = encode_VFR_HUD_v0_9(S.VFR_HUD);

		case 75
			%% Send a command with up to four parameters to the MAV
			p = encode_COMMAND_v0_9(S.COMMAND);

		case 76
			%% Report status of a command. Includes feedback wether the command was executed
			p = encode_COMMAND_ACK_v0_9(S.COMMAND_ACK);

		case 100
			%% Optical flow from a flow sensor (e.g. optical mouse sensor)
			p = encode_OPTICAL_FLOW_v0_9(S.OPTICAL_FLOW);

		case 140
			%% Object has been detected
			p = encode_OBJECT_DETECTION_EVENT_v0_9(S.OBJECT_DETECTION_EVENT);

		case 251
			p = encode_DEBUG_VECT_v0_9(S.DEBUG_VECT);
		...
		case 252
			...
			%% debug output.
			p = encode_NAMED_VALUE_FLOAT_v0_9(S.NAMED_VALUE_FLOAT);

		case 253
			%% Send a key-value pair as integer. The use of this message is discouraged for normal 
			%% packets, but a quite efficient way for testing new messages and getting experimental 
			%% debug output.
			p = encode_NAMED_VALUE_INT_v0_9(S.NAMED_VALUE_INT);
		...
		case 254
			...
			p = encode_STATUSTEXT_v0_9(S.STATUSTEXT);
		...
		case 255
			...
			p = encode_DEBUG_v0_9(S.DEBUG);

		case 150
			%% Offsets and calibrations values for hardware         sensors. This makes it easier 
			%% to debug the calibration process.
			p = encode_SENSOR_OFFSETS_v0_9(S.SENSOR_OFFSETS);

		case 151
			%% set the magnetometer offsets
			p = encode_SET_MAG_OFFSETS_v0_9(S.SET_MAG_OFFSETS);

		case 152
			%% state of APM memory
			p = encode_MEMINFO_v0_9(S.MEMINFO);

		case 153
			%% raw ADC output
			p = encode_AP_ADC_v0_9(S.AP_ADC);

		case 154
			%% Configure on-board Camera Control System.
			p = encode_DIGICAM_CONFIGURE_v0_9(S.DIGICAM_CONFIGURE);

		case 155
			%% Control on-board Camera Control System to take shots.
			p = encode_DIGICAM_CONTROL_v0_9(S.DIGICAM_CONTROL);

		case 156
			%% Message to configure a camera mount, directional antenna, etc.
			p = encode_MOUNT_CONFIGURE_v0_9(S.MOUNT_CONFIGURE);

		case 157
			%% Message to control a camera mount, directional antenna, etc.
			p = encode_MOUNT_CONTROL_v0_9(S.MOUNT_CONTROL);

		case 158
			%% Message with some status from APM to GCS about camera or antenna mount
			p = encode_MOUNT_STATUS_v0_9(S.MOUNT_STATUS);

		case 160
			%% A fence point. Used to set a point when from        GCS -> MAV. Also used to return 
			%% a point from MAV -> GCS
			p = encode_FENCE_POINT_v0_9(S.FENCE_POINT);

		case 161
			%% Request a current fence point from MAV
			p = encode_FENCE_FETCH_POINT_v0_9(S.FENCE_FETCH_POINT);

		case 162
			%% Status of geo-fencing. Sent in extended      status stream when fencing enabled
			p = encode_FENCE_STATUS_v0_9(S.FENCE_STATUS);
