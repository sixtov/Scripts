...
%%~ system.
function p = encode_POSITION_TARGET_GLOBAL_INT_v1_0(S)
	global pnum;
	if (isempty(pnum))
		pnum = 1;
	else
	...
	type = [ {'uint32'} {'uint8'} {'uint16'} {'int32'} {'int32'} {'single'} {'single'} {'single'} {'single'} {'single'} {'single'} {'single'} {'single'} {'single'} ];
	...
	%% Encode lat_int data field
	val = typecast(S.lat_int,'int32');
	val = reshape(val,1,length(val));
	p = [p typecast(val,'uint8')];

	%% Encode lon_int data field
	val = typecast(S.lon_int,'int32');
	val = reshape(val,1,length(val));
	p = [p typecast(val,'uint8')];
