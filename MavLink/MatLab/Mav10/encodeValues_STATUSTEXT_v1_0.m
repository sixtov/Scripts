...
function p = encodeValues_STATUSTEXT_v1_0(severity,text)
	S.severity = typecast(uint8(severity),'uint8');		% Severity of status. Relies on the definitions within RFC-5424. See enum MAV_SEVERITY.
	S.text = typecast(uint8(text),'uint8');		% Status text message, without null termination character
