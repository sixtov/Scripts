...
function p = encodeValues_HIL_SENSOR_v1_0(time_usec,xacc,yacc,zacc,xgyro,ygyro,zgyro,xmag,ymag,zmag,abs_pressure,diff_pressure,pressure_alt,temperature,fields_updated)
	...
	S.xgyro = typecast(single(xgyro),'single');		% Angular speed around X axis in body frame (rad / sec)
	S.ygyro = typecast(single(ygyro),'single');		% Angular speed around Y axis in body frame (rad / sec)
	S.zgyro = typecast(single(zgyro),'single');		% Angular speed around Z axis in body frame (rad / sec)
	...
	S.diff_pressure = typecast(single(diff_pressure),'single');		% Differential pressure (airspeed) in millibar
	...
	S.fields_updated = typecast(uint32(fields_updated),'uint32');		% Bitmask for fields that have updated since last message, bit 0 = xacc, bit 12: temperature
