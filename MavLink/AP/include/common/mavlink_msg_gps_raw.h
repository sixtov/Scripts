 ...
 */
static inline uint16_t mavlink_msg_gps_raw_pack(
	 uint8_t system_id
	...
	)
{
	...
	memcpy(_MAV_PAYLOAD(msg), buf, MAVLINK_MSG_ID_GPS_RAW_LEN);
#else
	...
	memcpy(_MAV_PAYLOAD(msg), &packet, MAVLINK_MSG_ID_GPS_RAW_LEN);
#endif
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_gps_raw_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
 ...
 */
static inline uint16_t mavlink_msg_gps_raw_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
	,const mavlink_gps_raw_t* gps_raw
	)
{
	return mavlink_msg_gps_raw_pack(
	 system_id
	,component_id
	,msg
	,gps_raw->usec
...
static inline void mavlink_msg_gps_raw_send(
...
static inline void mavlink_msg_wID_gps_raw_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint64_t usec
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_GPS_RAW, buf, MAVLINK_MSG_ID_GPS_RAW_LEN);
#else
	mavlink_gps_raw_t packet;
	packet.usec = usec;
	packet.fix_type = fix_type;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_GPS_RAW, (const char*)&packet, MAVLINK_MSG_ID_GPS_RAW_LEN);
#endif
}
