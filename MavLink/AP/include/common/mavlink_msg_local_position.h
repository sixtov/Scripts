 ...
 */
static inline uint16_t mavlink_msg_local_position_pack(
	 uint8_t system_id
	...
	)
{
	...
	memcpy(_MAV_PAYLOAD(msg), buf, MAVLINK_MSG_ID_LOCAL_POSITION_LEN);
#else
	...
	memcpy(_MAV_PAYLOAD(msg), &packet, MAVLINK_MSG_ID_LOCAL_POSITION_LEN);
#endif
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_local_position_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
static inline uint16_t mavlink_msg_local_position_encode(
	...
	,uint8_t component_id
	,mavlink_message_t* msg
	,const mavlink_local_position_t* local_position
	)
{
	return mavlink_msg_local_position_pack(
	 system_id
	,component_id
	,msg
	,local_position->usec
...
static inline void mavlink_msg_local_position_send(
...
static inline void mavlink_msg_wID_local_position_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint64_t usec
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_LOCAL_POSITION, buf, MAVLINK_MSG_ID_LOCAL_POSITION_LEN);
#else
	mavlink_local_position_t packet;
	packet.usec = usec;
	packet.x = x;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_LOCAL_POSITION, (const char*)&packet, MAVLINK_MSG_ID_LOCAL_POSITION_LEN);
#endif
}
