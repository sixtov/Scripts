 ...
 */
static inline uint16_t mavlink_msg_waypoint_clear_all_pack(
	 uint8_t system_id
	...
	)
{
	...
	memcpy(_MAV_PAYLOAD(msg), buf, MAVLINK_MSG_ID_WAYPOINT_CLEAR_ALL_LEN);
#else
	mavlink_waypoint_clear_all_t packet;
	packet.target_system = target_system;
	packet.target_component = target_component;

	memcpy(_MAV_PAYLOAD(msg), &packet, MAVLINK_MSG_ID_WAYPOINT_CLEAR_ALL_LEN);
#endif
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_waypoint_clear_all_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
static inline uint16_t mavlink_msg_waypoint_clear_all_encode(
	...
	,uint8_t component_id
	,mavlink_message_t* msg
	,const mavlink_waypoint_clear_all_t* waypoint_clear_all
	)
{
	return mavlink_msg_waypoint_clear_all_pack(
	 system_id
	,component_id
	,msg
	,waypoint_clear_all->target_system
	,waypoint_clear_all->target_component
	);
}
...
static inline void mavlink_msg_waypoint_clear_all_send(
...
static inline void mavlink_msg_wID_waypoint_clear_all_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint8_t target_system
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WAYPOINT_CLEAR_ALL, buf, MAVLINK_MSG_ID_WAYPOINT_CLEAR_ALL_LEN);
#else
	mavlink_waypoint_clear_all_t packet;
	packet.target_system = target_system;
	packet.target_component = target_component;

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WAYPOINT_CLEAR_ALL, (const char*)&packet, MAVLINK_MSG_ID_WAYPOINT_CLEAR_ALL_LEN);
#endif
}
