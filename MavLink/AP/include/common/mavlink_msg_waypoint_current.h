	...
	)
{
	...
	memcpy(_MAV_PAYLOAD(msg), buf, MAVLINK_MSG_ID_WAYPOINT_CURRENT_LEN);
#else
	mavlink_waypoint_current_t packet;
	packet.seq = seq;

	memcpy(_MAV_PAYLOAD(msg), &packet, MAVLINK_MSG_ID_WAYPOINT_CURRENT_LEN);
#endif
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_waypoint_current_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
 ...
 */
static inline uint16_t mavlink_msg_waypoint_current_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
	,const mavlink_waypoint_current_t* waypoint_current
	)
{
	return mavlink_msg_waypoint_current_pack(
	 system_id
	,component_id
	,msg
	,waypoint_current->seq
	);
}
...
static inline void mavlink_msg_waypoint_current_send(
...
static inline void mavlink_msg_wID_waypoint_current_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint16_t seq
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WAYPOINT_CURRENT, buf, MAVLINK_MSG_ID_WAYPOINT_CURRENT_LEN);
#else
	mavlink_waypoint_current_t packet;
	packet.seq = seq;

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WAYPOINT_CURRENT, (const char*)&packet, MAVLINK_MSG_ID_WAYPOINT_CURRENT_LEN);
#endif
}
