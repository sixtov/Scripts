 ...
 */
static inline uint16_t mavlink_msg_waypoint_count_pack(
	 uint8_t system_id
	...
	)
{
	...
	memcpy(_MAV_PAYLOAD(msg), buf, MAVLINK_MSG_ID_WAYPOINT_COUNT_LEN);
#else
	...
	memcpy(_MAV_PAYLOAD(msg), &packet, MAVLINK_MSG_ID_WAYPOINT_COUNT_LEN);
#endif
...
static inline uint16_t mavlink_msg_waypoint_count_encode(
	...
	,uint8_t component_id
	,mavlink_message_t* msg
	,const mavlink_waypoint_count_t* waypoint_count
	)
{
	return mavlink_msg_waypoint_count_pack(
	 system_id
	,component_id
	,msg
	,waypoint_count->target_system
	,waypoint_count->target_component
...
static inline void mavlink_msg_waypoint_count_send(
...
static inline void mavlink_msg_wID_waypoint_count_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint8_t target_system
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WAYPOINT_COUNT, buf, MAVLINK_MSG_ID_WAYPOINT_COUNT_LEN);
#else
	mavlink_waypoint_count_t packet;
	packet.target_system = target_system;
	packet.target_component = target_component;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WAYPOINT_COUNT, (const char*)&packet, MAVLINK_MSG_ID_WAYPOINT_COUNT_LEN);
#endif
}
