 ...
 */
static inline uint16_t mavlink_msg_set_nav_mode_pack(
	 uint8_t system_id
	...
	)
{
	...
	memcpy(_MAV_PAYLOAD(msg), buf, MAVLINK_MSG_ID_SET_NAV_MODE_LEN);
#else
	mavlink_set_nav_mode_t packet;
	packet.target = target;
	packet.nav_mode = nav_mode;

	memcpy(_MAV_PAYLOAD(msg), &packet, MAVLINK_MSG_ID_SET_NAV_MODE_LEN);
#endif
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_set_nav_mode_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
 ...
 */
static inline uint16_t mavlink_msg_set_nav_mode_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
	,const mavlink_set_nav_mode_t* set_nav_mode
	)
{
	return mavlink_msg_set_nav_mode_pack(
	 system_id
	,component_id
	,msg
	,set_nav_mode->target
...
static inline void mavlink_msg_set_nav_mode_send(
...
static inline void mavlink_msg_wID_set_nav_mode_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint8_t target
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_SET_NAV_MODE, buf, MAVLINK_MSG_ID_SET_NAV_MODE_LEN);
#else
	mavlink_set_nav_mode_t packet;
	packet.target = target;
	packet.nav_mode = nav_mode;

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_SET_NAV_MODE, (const char*)&packet, MAVLINK_MSG_ID_SET_NAV_MODE_LEN);
#endif
}
