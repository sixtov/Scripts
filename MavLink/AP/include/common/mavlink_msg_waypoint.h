 ...
 */
static inline uint16_t mavlink_msg_waypoint_pack(
	 uint8_t system_id
 ...
 */
static inline uint16_t mavlink_msg_waypoint_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
	,const mavlink_waypoint_t* waypoint
	)
{
	return mavlink_msg_waypoint_pack(
	 system_id
	,component_id
	,msg
...
static inline void mavlink_msg_waypoint_send(
...
static inline void mavlink_msg_wID_waypoint_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint8_t target_system
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WAYPOINT, (const char*)&packet, MAVLINK_MSG_ID_WAYPOINT_LEN);
#endif
}
