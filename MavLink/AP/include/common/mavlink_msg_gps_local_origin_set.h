 ...
 */
static inline uint16_t mavlink_msg_gps_local_origin_set_pack(
	 uint8_t system_id
	...
	)
{
	...
	memcpy(_MAV_PAYLOAD(msg), buf, MAVLINK_MSG_ID_GPS_LOCAL_ORIGIN_SET_LEN);
#else
	...
	memcpy(_MAV_PAYLOAD(msg), &packet, MAVLINK_MSG_ID_GPS_LOCAL_ORIGIN_SET_LEN);
#endif
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_gps_local_origin_set_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
static inline uint16_t mavlink_msg_gps_local_origin_set_encode(
	...
	,uint8_t component_id
	,mavlink_message_t* msg
	,const mavlink_gps_local_origin_set_t* gps_local_origin_set
	)
{
	return mavlink_msg_gps_local_origin_set_pack(
	 system_id
	,component_id
	,msg
	,gps_local_origin_set->latitude
	,gps_local_origin_set->longitude
	,gps_local_origin_set->altitude
	);
}
...
static inline void mavlink_msg_gps_local_origin_set_send(
...
static inline void mavlink_msg_wID_gps_local_origin_set_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,int32_t latitude
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_GPS_LOCAL_ORIGIN_SET, buf, MAVLINK_MSG_ID_GPS_LOCAL_ORIGIN_SET_LEN);
#else
	mavlink_gps_local_origin_set_t packet;
	packet.latitude = latitude;
	packet.longitude = longitude;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_GPS_LOCAL_ORIGIN_SET, (const char*)&packet, MAVLINK_MSG_ID_GPS_LOCAL_ORIGIN_SET_LEN);
#endif
}
