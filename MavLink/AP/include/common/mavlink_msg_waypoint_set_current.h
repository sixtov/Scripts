 ...
 */
static inline uint16_t mavlink_msg_waypoint_set_current_pack(
	 uint8_t system_id
	...
	)
{
	...
	memcpy(_MAV_PAYLOAD(msg), buf, MAVLINK_MSG_ID_WAYPOINT_SET_CURRENT_LEN);
#else
	...
	memcpy(_MAV_PAYLOAD(msg), &packet, MAVLINK_MSG_ID_WAYPOINT_SET_CURRENT_LEN);
#endif
...
static inline uint16_t mavlink_msg_waypoint_set_current_encode(
	...
	,uint8_t component_id
	,mavlink_message_t* msg
	,const mavlink_waypoint_set_current_t* waypoint_set_current
	)
{
	return mavlink_msg_waypoint_set_current_pack(
	 system_id
	,component_id
	,msg
	,waypoint_set_current->target_system
	,waypoint_set_current->target_component
	,waypoint_set_current->seq
	);
}
...
static inline void mavlink_msg_waypoint_set_current_send(
...
static inline void mavlink_msg_wID_waypoint_set_current_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint8_t target_system
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WAYPOINT_SET_CURRENT, buf, MAVLINK_MSG_ID_WAYPOINT_SET_CURRENT_LEN);
#else
	mavlink_waypoint_set_current_t packet;
	packet.target_system = target_system;
	packet.target_component = target_component;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WAYPOINT_SET_CURRENT, (const char*)&packet, MAVLINK_MSG_ID_WAYPOINT_SET_CURRENT_LEN);
#endif
}
