	...
	)
{
	...
	memcpy(_MAV_PAYLOAD(msg), buf, MAVLINK_MSG_ID_STATUSTEXT_LEN);
#else
	mavlink_statustext_t packet;
	packet.severity = severity;
	mav_array_memcpy(packet.text, text, sizeof(int8_t)*50);

	memcpy(_MAV_PAYLOAD(msg), &packet, MAVLINK_MSG_ID_STATUSTEXT_LEN);
#endif
 ...
 */
static inline int8_t mavlink_msg_statustext_get_text(const mavlink_message_t* msg, int8_t* text)
{
