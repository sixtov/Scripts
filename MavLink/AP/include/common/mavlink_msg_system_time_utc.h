 ...
 */
static inline uint16_t mavlink_msg_system_time_utc_pack(
	 uint8_t system_id
	...
	)
{
	...
	memcpy(_MAV_PAYLOAD(msg), buf, MAVLINK_MSG_ID_SYSTEM_TIME_UTC_LEN);
#else
	mavlink_system_time_utc_t packet;
	packet.utc_date = utc_date;
	packet.utc_time = utc_time;

	memcpy(_MAV_PAYLOAD(msg), &packet, MAVLINK_MSG_ID_SYSTEM_TIME_UTC_LEN);
#endif
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_system_time_utc_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
 ...
 */
static inline uint16_t mavlink_msg_system_time_utc_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
	,const mavlink_system_time_utc_t* system_time_utc
	)
{
	return mavlink_msg_system_time_utc_pack(
	 system_id
	,component_id
	,msg
	,system_time_utc->utc_date
	,system_time_utc->utc_time
	);
}
...
static inline void mavlink_msg_system_time_utc_send(
...
static inline void mavlink_msg_wID_system_time_utc_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint32_t utc_date
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_SYSTEM_TIME_UTC, buf, MAVLINK_MSG_ID_SYSTEM_TIME_UTC_LEN);
#else
	mavlink_system_time_utc_t packet;
	packet.utc_date = utc_date;
	packet.utc_time = utc_time;

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_SYSTEM_TIME_UTC, (const char*)&packet, MAVLINK_MSG_ID_SYSTEM_TIME_UTC_LEN);
#endif
}
