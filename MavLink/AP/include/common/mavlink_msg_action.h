 ...
 */
static inline uint16_t mavlink_msg_action_pack(
	 uint8_t system_id
	...
	)
{
	...
	memcpy(_MAV_PAYLOAD(msg), buf, MAVLINK_MSG_ID_ACTION_LEN);
#else
	...
	memcpy(_MAV_PAYLOAD(msg), &packet, MAVLINK_MSG_ID_ACTION_LEN);
#endif
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_action_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
 ...
 */
static inline uint16_t mavlink_msg_action_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
	,const mavlink_action_t* action
	)
{
	return mavlink_msg_action_pack(
	 system_id
	,component_id
	,msg
	,action->target
...
static inline void mavlink_msg_action_send(
...
static inline void mavlink_msg_wID_action_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint8_t target
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_ACTION, buf, MAVLINK_MSG_ID_ACTION_LEN);
#else
	mavlink_action_t packet;
	packet.target = target;
	packet.target_component = target_component;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_ACTION, (const char*)&packet, MAVLINK_MSG_ID_ACTION_LEN);
#endif
}
