 ...
 */
static inline uint16_t mavlink_msg_control_status_pack(
	 uint8_t system_id
	...
	,uint8_t ahrs_health
	,uint8_t control_att
	,uint8_t control_pos_xy
	,uint8_t control_pos_z
	,uint8_t control_pos_yaw
	)
{
	...
	memcpy(_MAV_PAYLOAD(msg), buf, MAVLINK_MSG_ID_CONTROL_STATUS_LEN);
#else
	...
	memcpy(_MAV_PAYLOAD(msg), &packet, MAVLINK_MSG_ID_CONTROL_STATUS_LEN);
#endif
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_control_status_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
 ...
 */
static inline uint16_t mavlink_msg_control_status_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
	,const mavlink_control_status_t* control_status
	)
{
	return mavlink_msg_control_status_pack(
	 system_id
	,component_id
	,msg
	,control_status->position_fix
	,control_status->vision_fix
	,control_status->gps_fix
	,control_status->ahrs_health
	,control_status->control_att
	,control_status->control_pos_xy
	,control_status->control_pos_z
	,control_status->control_pos_yaw
	);
}
...
static inline void mavlink_msg_control_status_send(
...
static inline void mavlink_msg_wID_control_status_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint8_t position_fix
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_CONTROL_STATUS, buf, MAVLINK_MSG_ID_CONTROL_STATUS_LEN);
#else
	mavlink_control_status_t packet;
	packet.position_fix = position_fix;
	packet.vision_fix = vision_fix;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_CONTROL_STATUS, (const char*)&packet, MAVLINK_MSG_ID_CONTROL_STATUS_LEN);
#endif
}
