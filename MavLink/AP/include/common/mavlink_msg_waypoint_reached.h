 ...
 */
static inline uint16_t mavlink_msg_waypoint_reached_pack(
	 uint8_t system_id
	...
	)
{
	...
	memcpy(_MAV_PAYLOAD(msg), buf, MAVLINK_MSG_ID_WAYPOINT_REACHED_LEN);
#else
	mavlink_waypoint_reached_t packet;
	packet.seq = seq;

	memcpy(_MAV_PAYLOAD(msg), &packet, MAVLINK_MSG_ID_WAYPOINT_REACHED_LEN);
#endif
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_waypoint_reached_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
 ...
 */
static inline uint16_t mavlink_msg_waypoint_reached_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
	,const mavlink_waypoint_reached_t* waypoint_reached
	)
{
	return mavlink_msg_waypoint_reached_pack(
	 system_id
	,component_id
	,msg
	,waypoint_reached->seq
	);
}
...
static inline void mavlink_msg_waypoint_reached_send(
...
static inline void mavlink_msg_wID_waypoint_reached_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint16_t seq
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WAYPOINT_REACHED, buf, MAVLINK_MSG_ID_WAYPOINT_REACHED_LEN);
#else
	mavlink_waypoint_reached_t packet;
	packet.seq = seq;

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WAYPOINT_REACHED, (const char*)&packet, MAVLINK_MSG_ID_WAYPOINT_REACHED_LEN);
#endif
}
