 ...
 */
static inline uint16_t mavlink_msg_waypoint_request_list_pack(
	 uint8_t system_id
	...
	)
{
	...
	memcpy(_MAV_PAYLOAD(msg), buf, MAVLINK_MSG_ID_WAYPOINT_REQUEST_LIST_LEN);
#else
	mavlink_waypoint_request_list_t packet;
	packet.target_system = target_system;
	packet.target_component = target_component;

	memcpy(_MAV_PAYLOAD(msg), &packet, MAVLINK_MSG_ID_WAYPOINT_REQUEST_LIST_LEN);
#endif
...
static inline uint16_t mavlink_msg_waypoint_request_list_encode(
	...
	,uint8_t component_id
	,mavlink_message_t* msg
	,const mavlink_waypoint_request_list_t* waypoint_request_list
	)
{
	return mavlink_msg_waypoint_request_list_pack(
	 system_id
	,component_id
	,msg
	,waypoint_request_list->target_system
	,waypoint_request_list->target_component
	);
}
...
static inline void mavlink_msg_waypoint_request_list_send(
...
static inline void mavlink_msg_wID_waypoint_request_list_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint8_t target_system
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WAYPOINT_REQUEST_LIST, buf, MAVLINK_MSG_ID_WAYPOINT_REQUEST_LIST_LEN);
#else
	mavlink_waypoint_request_list_t packet;
	packet.target_system = target_system;
	packet.target_component = target_component;

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WAYPOINT_REQUEST_LIST, (const char*)&packet, MAVLINK_MSG_ID_WAYPOINT_REQUEST_LIST_LEN);
#endif
}
