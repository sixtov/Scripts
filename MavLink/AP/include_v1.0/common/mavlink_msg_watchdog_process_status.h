...
#define MAVLINK_MSG_ID_WATCHDOG_PROCESS_STATUS_CRC 0
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_watchdog_process_status_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
 ...
 */
static inline uint16_t mavlink_msg_watchdog_process_status_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
...
{
	return mavlink_msg_watchdog_process_status_pack(
	 system_id
	,component_id
	,msg
	,watchdog_process_status->watchdog_id
	,watchdog_process_status->process_id
	,watchdog_process_status->state
	,watchdog_process_status->muted
	,watchdog_process_status->pid
	,watchdog_process_status->crashes
	);
}
 ...
 * @param watchdog_process_status C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_watchdog_process_status_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_watchdog_process_status_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_wID_watchdog_process_status_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint16_t watchdog_id
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WATCHDOG_PROCESS_STATUS, buf, MAVLINK_MSG_ID_WATCHDOG_PROCESS_STATUS_LEN, MAVLINK_MSG_ID_WATCHDOG_PROCESS_STATUS_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WATCHDOG_PROCESS_STATUS, buf, MAVLINK_MSG_ID_WATCHDOG_PROCESS_STATUS_LEN);
#endif
#else
	mavlink_watchdog_process_status_t packet;
	packet.watchdog_id = watchdog_id;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WATCHDOG_PROCESS_STATUS, (const char*)&packet, MAVLINK_MSG_ID_WATCHDOG_PROCESS_STATUS_LEN, MAVLINK_MSG_ID_WATCHDOG_PROCESS_STATUS_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WATCHDOG_PROCESS_STATUS, (const char*)&packet, MAVLINK_MSG_ID_WATCHDOG_PROCESS_STATUS_LEN);
#endif
#endif
}
