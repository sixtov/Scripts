...
#define MAVLINK_MSG_ID_PATTERN_DETECTED_CRC 0
#define MAVLINK_MSG_ID_190_CRC 0
...
{ \
  ...
  { \
    ...
    { "detected", NULL, MAVLINK_TYPE_UINT8_T, 0, 6, offsetof(mavlink_pattern_detected_t, detected) }, \
  } \
}
 ...
 */
static inline uint16_t mavlink_msg_pattern_detected_pack(
	 uint8_t system_id
	...
	)
{
	...
	_mav_put_uint8_t(buf, 6, detected);
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_pattern_detected_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
 ...
 */
static inline uint16_t mavlink_msg_pattern_detected_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
...
{
	return mavlink_msg_pattern_detected_pack(
	 system_id
	,component_id
	,msg
	,pattern_detected->type
	,pattern_detected->confidence
	,pattern_detected->file
 ...
 * @param pattern_detected C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_pattern_detected_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_pattern_detected_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_pattern_detected_send(
...
static inline void mavlink_msg_wID_pattern_detected_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint8_t type
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_PATTERN_DETECTED, buf, MAVLINK_MSG_ID_PATTERN_DETECTED_LEN, MAVLINK_MSG_ID_PATTERN_DETECTED_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_PATTERN_DETECTED, buf, MAVLINK_MSG_ID_PATTERN_DETECTED_LEN);
#endif
#else
	mavlink_pattern_detected_t packet;
	packet.type = type;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_PATTERN_DETECTED, (const char*)&packet, MAVLINK_MSG_ID_PATTERN_DETECTED_LEN, MAVLINK_MSG_ID_PATTERN_DETECTED_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_PATTERN_DETECTED, (const char*)&packet, MAVLINK_MSG_ID_PATTERN_DETECTED_LEN);
#endif
#endif
}
 ...
 */
static inline char mavlink_msg_pattern_detected_get_file(const mavlink_message_t* msg, char* file)
{
