...
#define MAVLINK_MSG_ID_ATTITUDE_CONTROL_CRC 0
 ...
 */
static inline uint16_t mavlink_msg_attitude_control_pack(
	 uint8_t system_id
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_attitude_control_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
	...
	)
{
	...
	,component_id
	,msg
	,attitude_control->target
	,attitude_control->roll
	,attitude_control->pitch
	,attitude_control->yaw
	,attitude_control->thrust
	,attitude_control->roll_manual
	,attitude_control->pitch_manual
	,attitude_control->yaw_manual
	,attitude_control->thrust_manual
	);
}
 ...
 * @param attitude_control C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_attitude_control_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_attitude_control_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_attitude_control_send(
...
static inline void mavlink_msg_wID_attitude_control_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint8_t target
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_ATTITUDE_CONTROL, buf, MAVLINK_MSG_ID_ATTITUDE_CONTROL_LEN, MAVLINK_MSG_ID_ATTITUDE_CONTROL_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_ATTITUDE_CONTROL, buf, MAVLINK_MSG_ID_ATTITUDE_CONTROL_LEN);
#endif
#else
	mavlink_attitude_control_t packet;
	packet.target = target;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_ATTITUDE_CONTROL, (const char*)&packet, MAVLINK_MSG_ID_ATTITUDE_CONTROL_LEN, MAVLINK_MSG_ID_ATTITUDE_CONTROL_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_ATTITUDE_CONTROL, (const char*)&packet, MAVLINK_MSG_ID_ATTITUDE_CONTROL_LEN);
#endif
#endif
}
