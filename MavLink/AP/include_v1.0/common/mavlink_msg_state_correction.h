...
#define MAVLINK_MSG_ID_STATE_CORRECTION_CRC 0
 ...
 */
static inline uint16_t mavlink_msg_state_correction_pack(
	 uint8_t system_id
 ...
 */
static inline uint16_t mavlink_msg_state_correction_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
	...
	)
{
	...
	,component_id
	,msg
	,state_correction->xErr
	,state_correction->yErr
	,state_correction->zErr
	,state_correction->rollErr
	,state_correction->pitchErr
	,state_correction->yawErr
	,state_correction->vxErr
	,state_correction->vyErr
	,state_correction->vzErr
	);
}
 ...
 * @param state_correction C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_state_correction_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_state_correction_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_state_correction_send(
...
static inline void mavlink_msg_wID_state_correction_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,float xErr
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_STATE_CORRECTION, buf, MAVLINK_MSG_ID_STATE_CORRECTION_LEN, MAVLINK_MSG_ID_STATE_CORRECTION_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_STATE_CORRECTION, buf, MAVLINK_MSG_ID_STATE_CORRECTION_LEN);
#endif
#else
	mavlink_state_correction_t packet;
	packet.xErr = xErr;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_STATE_CORRECTION, (const char*)&packet, MAVLINK_MSG_ID_STATE_CORRECTION_LEN, MAVLINK_MSG_ID_STATE_CORRECTION_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_STATE_CORRECTION, (const char*)&packet, MAVLINK_MSG_ID_STATE_CORRECTION_LEN);
#endif
#endif
}
