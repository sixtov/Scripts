...
{ 
  ...
  int16_t data;  ///< Terrain data in meters AMSL
  uint8_t gridbit[0];  ///< bit within the terrain request mask
} mavlink_terrain_data_t;

#define MAVLINK_MSG_ID_TERRAIN_DATA_LEN 43
#define MAVLINK_MSG_ID_134_LEN 43

#define MAVLINK_MSG_ID_TERRAIN_DATA_CRC 0
#define MAVLINK_MSG_ID_134_CRC 0
...
{ \
  ...
  { \
    ...
    { "gridbit", NULL, MAVLINK_TYPE_UINT8_T, 0, 12, offsetof(mavlink_terrain_data_t, gridbit) }, \
  } \
}
 ...
 */
static inline uint16_t mavlink_msg_terrain_data_pack(
	 uint8_t system_id
	...
	)
{
	...
	_mav_put_int16_t(buf, 10, data);
	_mav_put_uint8_t_array(buf, 12, gridbit, 0);
	...
	packet.data = data;
	mav_array_memcpy(packet.gridbit, gridbit, sizeof(uint8_t)*0);
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_terrain_data_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
/**
 * @brief Encode a terrain_data struct into a message
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param msg The MAVLink message to compress the data into
 ...
 */
static inline uint16_t mavlink_msg_terrain_data_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
 ...
 * @param terrain_data C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_terrain_data_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_terrain_data_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_terrain_data_send(
...
static inline void mavlink_msg_wID_terrain_data_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,int32_t lat
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_TERRAIN_DATA, buf, MAVLINK_MSG_ID_TERRAIN_DATA_LEN, MAVLINK_MSG_ID_TERRAIN_DATA_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_TERRAIN_DATA, buf, MAVLINK_MSG_ID_TERRAIN_DATA_LEN);
#endif
#else
	mavlink_terrain_data_t packet;
	packet.lat = lat;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_TERRAIN_DATA, (const char*)&packet, MAVLINK_MSG_ID_TERRAIN_DATA_LEN, MAVLINK_MSG_ID_TERRAIN_DATA_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_TERRAIN_DATA, (const char*)&packet, MAVLINK_MSG_ID_TERRAIN_DATA_LEN);
#endif
#endif
}
 ...
 */
static inline int16_t mavlink_msg_terrain_data_get_data(const mavlink_message_t* msg, int16_t* data)
{
