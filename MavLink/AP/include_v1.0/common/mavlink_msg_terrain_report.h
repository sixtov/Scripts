...
#define MAVLINK_MSG_ID_TERRAIN_REPORT_CRC 0
#define MAVLINK_MSG_ID_136_CRC 0
 ...
 */
static inline uint16_t mavlink_msg_terrain_report_pack(
	 uint8_t system_id
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_terrain_report_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
/**
 * @brief Encode a terrain_report struct into a message
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param msg The MAVLink message to compress the data into
 ...
 */
static inline uint16_t mavlink_msg_terrain_report_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
 ...
 * @param terrain_report C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_terrain_report_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_terrain_report_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_terrain_report_send(
...
static inline void mavlink_msg_wID_terrain_report_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,int32_t lat
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_TERRAIN_REPORT, buf, MAVLINK_MSG_ID_TERRAIN_REPORT_LEN, MAVLINK_MSG_ID_TERRAIN_REPORT_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_TERRAIN_REPORT, buf, MAVLINK_MSG_ID_TERRAIN_REPORT_LEN);
#endif
#else
	mavlink_terrain_report_t packet;
	packet.lat = lat;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_TERRAIN_REPORT, (const char*)&packet, MAVLINK_MSG_ID_TERRAIN_REPORT_LEN, MAVLINK_MSG_ID_TERRAIN_REPORT_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_TERRAIN_REPORT, (const char*)&packet, MAVLINK_MSG_ID_TERRAIN_REPORT_LEN);
#endif
#endif
}
