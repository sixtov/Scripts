...
#define MAVLINK_MSG_ID_DATA16_CRC 0
#define MAVLINK_MSG_ID_169_CRC 0
 ...
 */
static inline uint16_t mavlink_msg_data16_pack(
	 uint8_t system_id
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_data16_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
 ...
 */
static inline uint16_t mavlink_msg_data16_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
	,const mavlink_data16_t* data16
	)
{
 ...
 * @param data16 C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_data16_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_data16_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_data16_send(
...
static inline void mavlink_msg_wID_data16_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint8_t type
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_DATA16, buf, MAVLINK_MSG_ID_DATA16_LEN, MAVLINK_MSG_ID_DATA16_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_DATA16, buf, MAVLINK_MSG_ID_DATA16_LEN);
#endif
#else
	mavlink_data16_t packet;
	packet.type = type;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_DATA16, (const char*)&packet, MAVLINK_MSG_ID_DATA16_LEN, MAVLINK_MSG_ID_DATA16_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_DATA16, (const char*)&packet, MAVLINK_MSG_ID_DATA16_LEN);
#endif
#endif
}
 ...
 * @return raw data
 */
static inline uint8_t mavlink_msg_data16_get_data(const mavlink_message_t* msg, uint8_t* data)
{
