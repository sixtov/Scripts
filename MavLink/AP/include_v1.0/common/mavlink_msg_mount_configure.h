...
#define MAVLINK_MSG_ID_156_CRC 0
 ...
 */
static inline uint16_t mavlink_msg_mount_configure_pack(
	 uint8_t system_id
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_mount_configure_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
 ...
 */
static inline uint16_t mavlink_msg_mount_configure_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
	...
	)
{
	...
	,component_id
	,msg
	,mount_configure->target_system
	,mount_configure->target_component
	,mount_configure->mount_mode
	,mount_configure->stab_roll
	,mount_configure->stab_pitch
	,mount_configure->stab_yaw
	);
}
 ...
 * @param mount_configure C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_mount_configure_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
static inline void mavlink_msg_mount_configure_send(
...
static inline void mavlink_msg_wID_mount_configure_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint8_t target_system
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_MOUNT_CONFIGURE, buf, MAVLINK_MSG_ID_MOUNT_CONFIGURE_LEN, MAVLINK_MSG_ID_MOUNT_CONFIGURE_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_MOUNT_CONFIGURE, buf, MAVLINK_MSG_ID_MOUNT_CONFIGURE_LEN);
#endif
#else
	mavlink_mount_configure_t packet;
	packet.target_system = target_system;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_MOUNT_CONFIGURE, (const char*)&packet, MAVLINK_MSG_ID_MOUNT_CONFIGURE_LEN, MAVLINK_MSG_ID_MOUNT_CONFIGURE_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_MOUNT_CONFIGURE, (const char*)&packet, MAVLINK_MSG_ID_MOUNT_CONFIGURE_LEN);
#endif
#endif
}
