...
#define MAVLINK_MSG_ID_IMAGE_AVAILABLE_CRC 0
#define MAVLINK_MSG_ID_154_CRC 0
 ...
 */
static inline uint16_t mavlink_msg_image_available_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
...
{
	return mavlink_msg_image_available_pack(
	 system_id
	,component_id
	,msg
	,image_available->cam_id
	,image_available->cam_no
	,image_available->timestamp
	,image_available->valid_until
	,image_available->img_seq
	,image_available->img_buf_index
	,image_available->width
	,image_available->height
	,image_available->depth
	,image_available->channels
	,image_available->key
	,image_available->exposure
	,image_available->gain
	,image_available->roll
	,image_available->pitch
	,image_available->yaw
	,image_available->local_z
	,image_available->lat
	,image_available->lon
	,image_available->alt
	,image_available->ground_x
	,image_available->ground_y
	,image_available->ground_z
	);
}
 ...
 * @param image_available C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_image_available_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_image_available_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_image_available_send(
...
static inline void mavlink_msg_wID_image_available_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint64_t cam_id
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_IMAGE_AVAILABLE, buf, MAVLINK_MSG_ID_IMAGE_AVAILABLE_LEN, MAVLINK_MSG_ID_IMAGE_AVAILABLE_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_IMAGE_AVAILABLE, buf, MAVLINK_MSG_ID_IMAGE_AVAILABLE_LEN);
#endif
#else
	mavlink_image_available_t packet;
	packet.cam_id = cam_id;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_IMAGE_AVAILABLE, (const char*)&packet, MAVLINK_MSG_ID_IMAGE_AVAILABLE_LEN, MAVLINK_MSG_ID_IMAGE_AVAILABLE_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_IMAGE_AVAILABLE, (const char*)&packet, MAVLINK_MSG_ID_IMAGE_AVAILABLE_LEN);
#endif
#endif
}
