...
#define MAVLINK_MSG_ID_ENCAPSULATED_DATA_CRC 0
#define MAVLINK_MSG_ID_131_CRC 0
 ...
 */
static inline uint16_t mavlink_msg_encapsulated_data_pack(
	 uint8_t system_id
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_encapsulated_data_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
static inline uint16_t mavlink_msg_encapsulated_data_encode(
	...
	,uint8_t component_id
	,mavlink_message_t* msg
	,const mavlink_encapsulated_data_t* encapsulated_data
	)
{
 ...
 * @param encapsulated_data C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_encapsulated_data_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_encapsulated_data_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_encapsulated_data_send(
...
static inline void mavlink_msg_wID_encapsulated_data_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint16_t seqnr
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_ENCAPSULATED_DATA, buf, MAVLINK_MSG_ID_ENCAPSULATED_DATA_LEN, MAVLINK_MSG_ID_ENCAPSULATED_DATA_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_ENCAPSULATED_DATA, buf, MAVLINK_MSG_ID_ENCAPSULATED_DATA_LEN);
#endif
#else
	mavlink_encapsulated_data_t packet;
	packet.seqnr = seqnr;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_ENCAPSULATED_DATA, (const char*)&packet, MAVLINK_MSG_ID_ENCAPSULATED_DATA_LEN, MAVLINK_MSG_ID_ENCAPSULATED_DATA_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_ENCAPSULATED_DATA, (const char*)&packet, MAVLINK_MSG_ID_ENCAPSULATED_DATA_LEN);
#endif
#endif
}
 ...
 */
static inline uint8_t mavlink_msg_encapsulated_data_get_data(const mavlink_message_t* msg, uint8_t* data)
{
