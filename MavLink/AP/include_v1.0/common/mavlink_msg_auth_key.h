...
#define MAVLINK_MSG_ID_AUTH_KEY_CRC 0
#define MAVLINK_MSG_ID_7_CRC 0
 ...
 */
static inline uint16_t mavlink_msg_auth_key_pack(
	 uint8_t system_id
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_auth_key_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
 ...
 * @param auth_key C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_auth_key_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_auth_key_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_auth_key_send(
...
static inline void mavlink_msg_wID_auth_key_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,const char* key
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_AUTH_KEY, buf, MAVLINK_MSG_ID_AUTH_KEY_LEN, MAVLINK_MSG_ID_AUTH_KEY_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_AUTH_KEY, buf, MAVLINK_MSG_ID_AUTH_KEY_LEN);
#endif
#else
	mavlink_auth_key_t packet;
	mav_array_memcpy(packet.key, key, sizeof(char)*32);
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_AUTH_KEY, (const char*)&packet, MAVLINK_MSG_ID_AUTH_KEY_LEN, MAVLINK_MSG_ID_AUTH_KEY_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_AUTH_KEY, (const char*)&packet, MAVLINK_MSG_ID_AUTH_KEY_LEN);
#endif
#endif
}
 ...
 * @return key
 */
static inline char mavlink_msg_auth_key_get_key(const mavlink_message_t* msg, char* key)
{
