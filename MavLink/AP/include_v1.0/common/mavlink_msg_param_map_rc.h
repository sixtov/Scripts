...
{ 
  ...
  float param_value_min[0];  ///< Minimum param value. The protocol does not define if this overwrites an onboard minimum value. (Depends on implementation)
  ...
  char param_id;  ///< Onboard parameter id, terminated by NULL if the length is less than 16 human-readable chars and WITHOUT null termination (NULL) byte if the length is exactly 16 chars - applications have to provide 16+1 bytes storage if the ID is stored as string
...
#define MAVLINK_MSG_ID_50_CRC 0
...
{ \
  ...
  { \
    ...
    { "parameter_rc_channel_index", NULL, MAVLINK_TYPE_UINT8_T, 0, 21, offsetof(mavlink_param_map_rc_t, parameter_rc_channel_index) }, \
  } \
}
 ...
 */
static inline uint16_t mavlink_msg_param_map_rc_pack(
	 uint8_t system_id
	...
	)
{
	...
	_mav_put_float_array(buf, 8, param_value_min, 0);
	...
	_mav_put_char(buf, 20, param_id);
	_mav_put_uint8_t(buf, 21, parameter_rc_channel_index);
	...
	packet.scale = scale;
	mav_array_memcpy(packet.param_value_min, param_value_min, sizeof(float)*0);
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_param_map_rc_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
/**
 * @brief Encode a param_map_rc struct into a message
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param msg The MAVLink message to compress the data into
 ...
 */
static inline uint16_t mavlink_msg_param_map_rc_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
 ...
 * @param param_map_rc C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_param_map_rc_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_param_map_rc_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_param_map_rc_send(
...
static inline void mavlink_msg_wID_param_map_rc_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint8_t target_system
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_PARAM_MAP_RC, buf, MAVLINK_MSG_ID_PARAM_MAP_RC_LEN, MAVLINK_MSG_ID_PARAM_MAP_RC_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_PARAM_MAP_RC, buf, MAVLINK_MSG_ID_PARAM_MAP_RC_LEN);
#endif
#else
	mavlink_param_map_rc_t packet;
	packet.target_system = target_system;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_PARAM_MAP_RC, (const char*)&packet, MAVLINK_MSG_ID_PARAM_MAP_RC_LEN, MAVLINK_MSG_ID_PARAM_MAP_RC_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_PARAM_MAP_RC, (const char*)&packet, MAVLINK_MSG_ID_PARAM_MAP_RC_LEN);
#endif
#endif
}
 ...
 */
static inline char mavlink_msg_param_map_rc_get_param_id(const mavlink_message_t* msg, char* param_id)
{
