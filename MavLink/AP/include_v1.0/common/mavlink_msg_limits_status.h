...
#define MAVLINK_MSG_ID_LIMITS_STATUS_CRC 0
#define MAVLINK_MSG_ID_167_CRC 0
 ...
 */
static inline uint16_t mavlink_msg_limits_status_pack(
	 uint8_t system_id
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_limits_status_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
 ...
 */
static inline uint16_t mavlink_msg_limits_status_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
	...
	)
{
	...
	,component_id
	,msg
	,limits_status->limits_state
	,limits_status->last_trigger
	,limits_status->last_action
	,limits_status->last_recovery
	,limits_status->last_clear
	,limits_status->breach_count
	,limits_status->mods_enabled
	,limits_status->mods_required
	,limits_status->mods_triggered
	);
}
 ...
 * @param limits_status C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_limits_status_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_limits_status_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_wID_limits_status_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint8_t limits_state
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_LIMITS_STATUS, buf, MAVLINK_MSG_ID_LIMITS_STATUS_LEN, MAVLINK_MSG_ID_LIMITS_STATUS_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_LIMITS_STATUS, buf, MAVLINK_MSG_ID_LIMITS_STATUS_LEN);
#endif
#else
	mavlink_limits_status_t packet;
	packet.limits_state = limits_state;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_LIMITS_STATUS, (const char*)&packet, MAVLINK_MSG_ID_LIMITS_STATUS_LEN, MAVLINK_MSG_ID_LIMITS_STATUS_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_LIMITS_STATUS, (const char*)&packet, MAVLINK_MSG_ID_LIMITS_STATUS_LEN);
#endif
#endif
}
