...
{ 
  uint64_t time_usec[0];  ///< Timestamp
  float x;  ///< x
  float y;  ///< y
  float z;  ///< z
  char name;  ///< Name
...
#define MAVLINK_MSG_ID_DEBUG_VECT_CRC 0
#define MAVLINK_MSG_ID_250_CRC 0
	...
	)
{
	...
	_mav_put_uint64_t_array(buf, 0, time_usec, 0);
	_mav_put_float(buf, 8, x);
	_mav_put_float(buf, 12, y);
	_mav_put_float(buf, 16, z);
	_mav_put_char(buf, 20, name);

	memcpy(_MAV_PAYLOAD_NON_CONST(msg), buf, MAVLINK_MSG_ID_DEBUG_VECT_LEN);
#else
	mavlink_debug_vect_t packet;
	mav_array_memcpy(packet.time_usec, time_usec, sizeof(uint64_t)*0);
	packet.x = x;
	packet.y = y;
	packet.z = z;
	packet.name = name;
 ...
 * @param debug_vect C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_debug_vect_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_debug_vect_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_DEBUG_VECT, buf, MAVLINK_MSG_ID_DEBUG_VECT_LEN, MAVLINK_MSG_ID_DEBUG_VECT_CRC);
#else
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_DEBUG_VECT, (const char*)&packet, MAVLINK_MSG_ID_DEBUG_VECT_LEN, MAVLINK_MSG_ID_DEBUG_VECT_CRC);
#else
