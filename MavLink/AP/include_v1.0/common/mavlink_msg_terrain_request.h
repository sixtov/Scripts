...
#define MAVLINK_MSG_ID_TERRAIN_REQUEST_CRC 0
#define MAVLINK_MSG_ID_133_CRC 0
...
{ \
  ...
  4, \
  { \
    { "mask", NULL, MAVLINK_TYPE_UINT64_T, 0, 0, offsetof(mavlink_terrain_request_t, mask) }, \
 ...
 */
static inline uint16_t mavlink_msg_terrain_request_pack(
	 uint8_t system_id
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_terrain_request_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
/**
 * @brief Encode a terrain_request struct into a message
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param msg The MAVLink message to compress the data into
 ...
 */
static inline uint16_t mavlink_msg_terrain_request_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
 ...
 * @param terrain_request C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_terrain_request_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_terrain_request_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_terrain_request_send(
...
static inline void mavlink_msg_wID_terrain_request_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,int32_t lat
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_TERRAIN_REQUEST, buf, MAVLINK_MSG_ID_TERRAIN_REQUEST_LEN, MAVLINK_MSG_ID_TERRAIN_REQUEST_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_TERRAIN_REQUEST, buf, MAVLINK_MSG_ID_TERRAIN_REQUEST_LEN);
#endif
#else
	mavlink_terrain_request_t packet;
	packet.lat = lat;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_TERRAIN_REQUEST, (const char*)&packet, MAVLINK_MSG_ID_TERRAIN_REQUEST_LEN, MAVLINK_MSG_ID_TERRAIN_REQUEST_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_TERRAIN_REQUEST, (const char*)&packet, MAVLINK_MSG_ID_TERRAIN_REQUEST_LEN);
#endif
#endif
}
