...
#define MAVLINK_MSG_ID_DATA_STREAM_CRC 0
#define MAVLINK_MSG_ID_67_CRC 0
...
{ \
  ...
  3, \
  { \
    { "stream_id", NULL, MAVLINK_TYPE_UINT8_T, 0, 0, offsetof(mavlink_data_stream_t, stream_id) }, \
    { "message_rate", NULL, MAVLINK_TYPE_UINT16_T, 0, 1, offsetof(mavlink_data_stream_t, message_rate) }, \
 ...
 */
static inline uint16_t mavlink_msg_data_stream_pack(
	 uint8_t system_id
	...
	)
{
	...
	_mav_put_uint8_t(buf, 0, stream_id);
	_mav_put_uint16_t(buf, 1, message_rate);
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_data_stream_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
 ...
 */
static inline uint16_t mavlink_msg_data_stream_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
 ...
 * @param data_stream C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_data_stream_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_data_stream_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_data_stream_send(
...
static inline void mavlink_msg_wID_data_stream_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint8_t stream_id
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_DATA_STREAM, buf, MAVLINK_MSG_ID_DATA_STREAM_LEN, MAVLINK_MSG_ID_DATA_STREAM_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_DATA_STREAM, buf, MAVLINK_MSG_ID_DATA_STREAM_LEN);
#endif
#else
	mavlink_data_stream_t packet;
	packet.stream_id = stream_id;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_DATA_STREAM, (const char*)&packet, MAVLINK_MSG_ID_DATA_STREAM_LEN, MAVLINK_MSG_ID_DATA_STREAM_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_DATA_STREAM, (const char*)&packet, MAVLINK_MSG_ID_DATA_STREAM_LEN);
#endif
#endif
}
