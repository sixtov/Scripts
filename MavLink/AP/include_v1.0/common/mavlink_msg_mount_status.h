...
#define MAVLINK_MSG_ID_MOUNT_STATUS_CRC 0
#define MAVLINK_MSG_ID_158_CRC 0
 ...
 */
static inline uint16_t mavlink_msg_mount_status_pack(
	 uint8_t system_id
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_mount_status_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
 ...
 */
static inline uint16_t mavlink_msg_mount_status_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
	,const mavlink_mount_status_t* mount_status
	)
{
	...
	,component_id
	,msg
	,mount_status->target_system
	,mount_status->target_component
	,mount_status->pointing_a
	,mount_status->pointing_b
	,mount_status->pointing_c
	);
}
 ...
 * @param mount_status C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_mount_status_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_mount_status_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_mount_status_send(
...
static inline void mavlink_msg_wID_mount_status_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint8_t target_system
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_MOUNT_STATUS, buf, MAVLINK_MSG_ID_MOUNT_STATUS_LEN, MAVLINK_MSG_ID_MOUNT_STATUS_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_MOUNT_STATUS, buf, MAVLINK_MSG_ID_MOUNT_STATUS_LEN);
#endif
#else
	mavlink_mount_status_t packet;
	packet.target_system = target_system;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_MOUNT_STATUS, (const char*)&packet, MAVLINK_MSG_ID_MOUNT_STATUS_LEN, MAVLINK_MSG_ID_MOUNT_STATUS_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_MOUNT_STATUS, (const char*)&packet, MAVLINK_MSG_ID_MOUNT_STATUS_LEN);
#endif
#endif
}
