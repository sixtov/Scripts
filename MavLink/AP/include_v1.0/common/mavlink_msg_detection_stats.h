...
#define MAVLINK_MSG_ID_DETECTION_STATS_CRC 0
#define MAVLINK_MSG_ID_205_CRC 0
 ...
 */
static inline uint16_t mavlink_msg_detection_stats_pack(
	 uint8_t system_id
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_detection_stats_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
/**
 * @brief Encode a detection_stats struct into a message
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param msg The MAVLink message to compress the data into
 ...
 */
static inline uint16_t mavlink_msg_detection_stats_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
...
{
	return mavlink_msg_detection_stats_pack(
	 system_id
	,component_id
	,msg
	,detection_stats->detections
	,detection_stats->cluster_iters
	,detection_stats->best_score
	,detection_stats->best_lat
	,detection_stats->best_lon
	,detection_stats->best_alt
	,detection_stats->best_detection_id
	,detection_stats->best_cluster_id
	,detection_stats->best_cluster_iter_id
	,detection_stats->images_done
	,detection_stats->images_todo
	,detection_stats->fps
	);
}
 ...
 * @param detection_stats C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_detection_stats_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_detection_stats_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_detection_stats_send(
...
static inline void mavlink_msg_wID_detection_stats_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint32_t detections
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_DETECTION_STATS, buf, MAVLINK_MSG_ID_DETECTION_STATS_LEN, MAVLINK_MSG_ID_DETECTION_STATS_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_DETECTION_STATS, buf, MAVLINK_MSG_ID_DETECTION_STATS_LEN);
#endif
#else
	mavlink_detection_stats_t packet;
	packet.detections = detections;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_DETECTION_STATS, (const char*)&packet, MAVLINK_MSG_ID_DETECTION_STATS_LEN, MAVLINK_MSG_ID_DETECTION_STATS_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_DETECTION_STATS, (const char*)&packet, MAVLINK_MSG_ID_DETECTION_STATS_LEN);
#endif
#endif
}
