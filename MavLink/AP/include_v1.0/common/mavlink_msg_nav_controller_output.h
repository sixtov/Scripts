...
{ \
  ...
  { \
    ...
    { "nav_wptrad", NULL, MAVLINK_TYPE_FLOAT, 0, 26, offsetof(mavlink_nav_controller_output_t, nav_wptrad) }, \
  } \
}
	...
	)
{
	...
	_mav_put_float(buf, 26, nav_wptrad);
 ...
 */
static inline uint16_t mavlink_msg_nav_controller_output_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
 ...
 * @param nav_controller_output C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_nav_controller_output_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_nav_controller_output_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_nav_controller_output_send(
...
static inline void mavlink_msg_wID_nav_controller_output_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,float nav_roll
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_NAV_CONTROLLER_OUTPUT, (const char*)&packet, MAVLINK_MSG_ID_NAV_CONTROLLER_OUTPUT_LEN);
#endif
#endif
}
