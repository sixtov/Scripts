...
#define MAVLINK_MSG_ID_MOUNT_CONTROL_CRC 0
#define MAVLINK_MSG_ID_157_CRC 0
 ...
 */
static inline uint16_t mavlink_msg_mount_control_pack(
	 uint8_t system_id
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_mount_control_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
 ...
 */
static inline uint16_t mavlink_msg_mount_control_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
	,const mavlink_mount_control_t* mount_control
	)
{
	return mavlink_msg_mount_control_pack(
	 system_id
	,component_id
	,msg
	,mount_control->target_system
	,mount_control->target_component
	,mount_control->input_a
	,mount_control->input_b
	,mount_control->input_c
	,mount_control->save_position
	);
}
 ...
 * @param mount_control C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_mount_control_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_mount_control_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_mount_control_send(
...
static inline void mavlink_msg_wID_mount_control_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint8_t target_system
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_MOUNT_CONTROL, buf, MAVLINK_MSG_ID_MOUNT_CONTROL_LEN, MAVLINK_MSG_ID_MOUNT_CONTROL_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_MOUNT_CONTROL, buf, MAVLINK_MSG_ID_MOUNT_CONTROL_LEN);
#endif
#else
	mavlink_mount_control_t packet;
	packet.target_system = target_system;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_MOUNT_CONTROL, (const char*)&packet, MAVLINK_MSG_ID_MOUNT_CONTROL_LEN, MAVLINK_MSG_ID_MOUNT_CONTROL_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_MOUNT_CONTROL, (const char*)&packet, MAVLINK_MSG_ID_MOUNT_CONTROL_LEN);
#endif
#endif
}
