...
#define MAVLINK_MSG_ID_LOG_DATA_CRC 0
#define MAVLINK_MSG_ID_120_CRC 0
...
{ \
  ...
  4, \
  { \
    { "id", NULL, MAVLINK_TYPE_UINT16_T, 0, 0, offsetof(mavlink_log_data_t, id) }, \
    { "ofs", NULL, MAVLINK_TYPE_UINT32_T, 0, 2, offsetof(mavlink_log_data_t, ofs) }, \
 ...
 */
static inline uint16_t mavlink_msg_log_data_pack(
	 uint8_t system_id
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_log_data_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
/**
 * @brief Encode a log_data struct into a message
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param msg The MAVLink message to compress the data into
 ...
 */
static inline uint16_t mavlink_msg_log_data_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
 ...
 * @param log_data C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_log_data_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_log_data_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_log_data_send(
...
static inline void mavlink_msg_wID_log_data_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint16_t id
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_LOG_DATA, buf, MAVLINK_MSG_ID_LOG_DATA_LEN, MAVLINK_MSG_ID_LOG_DATA_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_LOG_DATA, buf, MAVLINK_MSG_ID_LOG_DATA_LEN);
#endif
#else
	mavlink_log_data_t packet;
	packet.id = id;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_LOG_DATA, (const char*)&packet, MAVLINK_MSG_ID_LOG_DATA_LEN, MAVLINK_MSG_ID_LOG_DATA_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_LOG_DATA, (const char*)&packet, MAVLINK_MSG_ID_LOG_DATA_LEN);
#endif
#endif
}
 ...
 * @return log data
 */
static inline uint8_t mavlink_msg_log_data_get_data(const mavlink_message_t* msg, uint8_t* data)
{
