...
{ 
  ...
  float vy;  ///< Y Speed
  float vz;  ///< Z Speed
  float covariance;  ///< Covariance matrix (first six entries are the first ROW, next six entries are the second row, etc.)
  uint8_t estimator_type[0];  ///< Class id of the estimator this estimate originated from.
} mavlink_local_position_ned_cov_t;

#define MAVLINK_MSG_ID_LOCAL_POSITION_NED_COV_LEN 181
#define MAVLINK_MSG_ID_64_LEN 181

#define MAVLINK_MSG_ID_LOCAL_POSITION_NED_COV_CRC 0
...
{ \
  ...
  { \
    ...
    { "estimator_type", NULL, MAVLINK_TYPE_UINT8_T, 0, 40, offsetof(mavlink_local_position_ned_cov_t, estimator_type) }, \
  } \
}
 ...
 */
static inline uint16_t mavlink_msg_local_position_ned_cov_pack(
	 uint8_t system_id
	...
	)
{
	...
	_mav_put_float(buf, 36, covariance);
	_mav_put_uint8_t_array(buf, 40, estimator_type, 0);
	...
	mav_array_memcpy(packet.estimator_type, estimator_type, sizeof(uint8_t)*0);
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_local_position_ned_cov_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
/**
 * @brief Encode a local_position_ned_cov struct into a message
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param msg The MAVLink message to compress the data into
 ...
 */
static inline uint16_t mavlink_msg_local_position_ned_cov_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
...
{
	return mavlink_msg_local_position_ned_cov_pack(
	 system_id
	,component_id
	,msg
	...
	,local_position_ned_cov->covariance
	);
}
 ...
 * @param local_position_ned_cov C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_local_position_ned_cov_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_local_position_ned_cov_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_local_position_ned_cov_send(
...
static inline void mavlink_msg_wID_local_position_ned_cov_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint32_t time_boot_ms
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_LOCAL_POSITION_NED_COV, buf, MAVLINK_MSG_ID_LOCAL_POSITION_NED_COV_LEN, MAVLINK_MSG_ID_LOCAL_POSITION_NED_COV_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_LOCAL_POSITION_NED_COV, buf, MAVLINK_MSG_ID_LOCAL_POSITION_NED_COV_LEN);
#endif
#else
	mavlink_local_position_ned_cov_t packet;
	packet.time_boot_ms = time_boot_ms;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_LOCAL_POSITION_NED_COV, (const char*)&packet, MAVLINK_MSG_ID_LOCAL_POSITION_NED_COV_LEN, MAVLINK_MSG_ID_LOCAL_POSITION_NED_COV_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_LOCAL_POSITION_NED_COV, (const char*)&packet, MAVLINK_MSG_ID_LOCAL_POSITION_NED_COV_LEN);
#endif
#endif
}
 ...
 */
static inline float mavlink_msg_local_position_ned_cov_get_covariance(const mavlink_message_t* msg, float* covariance)
{
	return _MAV_RETURN_float_array(msg,covariance,36,37);
}
