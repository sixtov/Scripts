...
#define MAVLINK_MSG_ID_TERRAIN_CHECK_CRC 0
#define MAVLINK_MSG_ID_135_CRC 0
 ...
 */
static inline uint16_t mavlink_msg_terrain_check_pack(
	 uint8_t system_id
...
/**
 * @brief Encode a terrain_check struct into a message
 *
 * @param system_id ID of this system
 * @param component_id ID of this component (e.g. 200 for IMU)
 * @param msg The MAVLink message to compress the data into
...
{
	return mavlink_msg_terrain_check_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_terrain_check_send(
...
static inline void mavlink_msg_wID_terrain_check_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,int32_t lat
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_TERRAIN_CHECK, buf, MAVLINK_MSG_ID_TERRAIN_CHECK_LEN, MAVLINK_MSG_ID_TERRAIN_CHECK_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_TERRAIN_CHECK, buf, MAVLINK_MSG_ID_TERRAIN_CHECK_LEN);
#endif
#else
	mavlink_terrain_check_t packet;
	packet.lat = lat;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_TERRAIN_CHECK, (const char*)&packet, MAVLINK_MSG_ID_TERRAIN_CHECK_LEN, MAVLINK_MSG_ID_TERRAIN_CHECK_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_TERRAIN_CHECK, (const char*)&packet, MAVLINK_MSG_ID_TERRAIN_CHECK_LEN);
#endif
#endif
}
