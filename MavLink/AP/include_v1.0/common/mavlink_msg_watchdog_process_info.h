...
{ 
  ...
  uint16_t process_id[0];  ///< Process ID
  char name[100];  ///< Process name
  char arguments;  ///< Process arguments
} mavlink_watchdog_process_info_t;

#define MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_LEN 255
#define MAVLINK_MSG_ID_181_LEN 255

#define MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_CRC 0
#define MAVLINK_MSG_ID_181_CRC 0
...
{ \
  ...
  { \
    ...
    { "arguments", NULL, MAVLINK_TYPE_CHAR, 147, 9, offsetof(mavlink_watchdog_process_info_t, arguments) }, \
  } \
}
 ...
 */
static inline uint16_t mavlink_msg_watchdog_process_info_pack(
	 uint8_t system_id
	...
	)
{
	...
	_mav_put_uint16_t_array(buf, 6, process_id, 0);
	_mav_put_char_array(buf, 8, name, 100);
	_mav_put_char(buf, 9, arguments);
	...
	mav_array_memcpy(packet.process_id, process_id, sizeof(uint16_t)*0);
 ...
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_watchdog_process_info_pack_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
 ...
 */
static inline uint16_t mavlink_msg_watchdog_process_info_encode(
	 uint8_t system_id
	,uint8_t component_id
	,mavlink_message_t* msg
	,const mavlink_watchdog_process_info_t* watchdog_process_info
	)
{
	return mavlink_msg_watchdog_process_info_pack(
	 system_id
	,component_id
	,msg
	,watchdog_process_info->watchdog_id
	,watchdog_process_info->process_id
	,watchdog_process_info->name
	,watchdog_process_info->arguments
	,watchdog_process_info->timeout
	);
}
 ...
 * @param watchdog_process_info C-struct to read the message contents from
 */
static inline uint16_t mavlink_msg_watchdog_process_info_encode_chan(
	 uint8_t system_id
	,uint8_t component_id
	,uint8_t chan
	,mavlink_message_t* msg
...
{
	return mavlink_msg_watchdog_process_info_pack_chan(
	 system_id
	,component_id
	,chan
	,msg
...
static inline void mavlink_msg_watchdog_process_info_send(
...
static inline void mavlink_msg_wID_watchdog_process_info_send(
	 mavlink_channel_t chan
	,uint8_t sID
	,uint8_t cID
	,uint16_t watchdog_id
	...
	)
{
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO, buf, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_LEN, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO, buf, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_LEN);
#endif
#else
	mavlink_watchdog_process_info_t packet;
	packet.watchdog_id = watchdog_id;
	...
	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO, (const char*)&packet, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_LEN, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_CRC);
#else

	_mav_wID_finalize_message_chan_send(chan, sID, cID, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO, (const char*)&packet, MAVLINK_MSG_ID_WATCHDOG_PROCESS_INFO_LEN);
#endif
#endif
}
 ...
 * @return Process name
 */
static inline char mavlink_msg_watchdog_process_info_get_name(const mavlink_message_t* msg, char* name)
{
 ...
 */
static inline char mavlink_msg_watchdog_process_info_get_arguments(const mavlink_message_t* msg, char* arguments)
{
	return _MAV_RETURN_char_array(msg,arguments,147,5);
}
